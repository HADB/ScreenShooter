...
$(document).ready(function () {
    ...
    chrome.windows.getCurrent(function (a) {
        chrome.tabs.getSelected(a.id,
        function (b) {
            chrome.browserAction.getBadgeText({
                tabId: b.id
            },
            function (b) {
                "New" == b && (chrome.browserAction.setBadgeText({
                    text: ""
                }), chrome.tabs.create({
                    url: "http://www.appchangelog.com/log/19/Awesome-Screenshot:-Capture-&-Annotate/Awesome-screenshot-for-iOS-8"
                }), localStorage.isClickedOnNew = "true", a.close())
            })
        })
    });
    var e, f = !0;
    b(),
    a(),
    localStorage.capture_desktop && "false" != localStorage.capture_desktop || $("#desktop").append('<span class="tip">(Premium feature)</span>').css({
        color: "#909090"
    }),
    chrome.windows.getCurrent(function (a) {
        chrome.tabs.getSelected(a.id,
        function (a) {
            ...
            e = a.url;
            var b = e.match(/https?:\/\/*\/*/gi); (null == b || e.match(/https:\/\/chrome.google.com\/extensions/i)) && $("#entire, #selected, #delay-capture").attr({
                title: chrome.i18n.getMessage("disableEntireTitle")
            }).css({
                color: "#909090"
            }).unbind("click"),
            "complete" != a.status && ($("#selected").attr({
                title: "Page still loading! Please wait."
            }).css({
                color: "#909090"
            }), f = !1),
            /http|https|file|ftp/.test(e.slice(0, 5)) || $("#visible").css({
                color: "#909090"
            }).unbind("click")
        })
    }),
    ...
    $("a").click(function () {
        ...
        if ("visible" == a && (-1 != navigator.appVersion.indexOf("Chrome/11") && /^file:\/\/*/.test(b) ? alert("You can't capture local page's in Chrome beta!") : d({
            action: a
        }), window.close()), "delay-capture" == a && (-1 != navigator.appVersion.indexOf("Chrome/11") && /^file:\/\/*/.test(b) ? alert("You can't capture local page's in Chrome beta!") : chrome.tabs.getSelected(null,
        function (b) {
        ...
        "desktop" == a && (localStorage.capture_desktop && "false" != localStorage.capture_desktop ? Bg.beginDesktop() : chrome.tabs.create({
            url: chrome.extension.getURL("") + "purchase.html"
        }), window.close()),
        "donate" == a && (chrome.tabs.create({
            url: chrome.extension.getURL("") + "purchase.html"
        }), window.close())
    })
});