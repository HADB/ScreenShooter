...
function disablePriceCompare() {
    ...
    "never" !== localStorage.show_feature_bar && (localStorage.show_feature_bar = "true"),
    refreshOptions()
}
...
function newTab() {
    ...
    dataURL ? ("selected" == menuType && sendRequest("tab", tabid, {
...
function prepareImage() {
    ...
    g = function () {
        function h(a, c, e, f, g, i, j, k, m) {
            j = r * b,
            r == u - 1 && (e = b - lastH, g = m = lastH),
            console.log(r, n, u - 1),
            $("#temp_image").attr({
                src: a
            }).load(function () {
                $(this).unbind("load"),
                d.drawImage(this, c, e, f, g, i, j, k, m),
                ++r > u - 1 ? (console.log("nextCol"), l()) : (console.log("PrepareV"), h(q[++n], c, e, f, g, i, j, k, m))
            })
        }
        function i(b, c, e, f, g, h, k, l, m) {
            h = j * a,
            j == t - 1 && (c = a - lastW, f = l = lastW),
            $("#temp_image").attr({
                src: b
            }).load(function () {
                $(this).unbind("load"),
                d.drawImage(this, c, e, f, g, h, k, l, m),
                t - 1 > j && i(q[++j], c, e, f, g, h, k, l, m)
            })
        }
        function k() {
            $(c).attr({
        ...
        function l() {
            if (++j > t - 1) {
                ...
                return "jpg" == localStorage.format ? tempDataUrl = c.toDataURL("image/jpeg") : "png" == localStorage.format && (tempDataUrl = c.toDataURL()),
                console.log("final", tempDataUrl),
                d.AutoSave(tempDataUrl, tabtitle.replace(/[#$~!@%^&*();'"?><\[\]{}\|,:\/=+-]/g, " "), localStorage.savePath),
...
function onLicenseUpdate(a) {
    function b() {
        if (d > 0) {
            for (var a = 0; d > a; a++) if ("capture_desktop_window" === c[a].sku) return !0;
            return !1
        }
        return !1
    }
...
chrome.extension.onRequest.addListener(function (a, b, c) {
    ...
    switch (console.log(b.tab), chrome.tabs.getSelected(null,
    function (a) {
        ...
        case "visible":
            "selected" == menuType && (type = "visible", centerW = a.centerW, centerH = a.centerH),
            captureVisible(),
            sendRequest("tab", tabid, {
                action: "restorebar"
            });
        ...
        case "ready":
            ...
            break;
        case "copy":
            chrome.experimental.clipboard.executeCopy(tabid,
            function () {
                alert("copied")
            });
            break;
        ...
        case "newtip":
            if (!localStorage.version || localStorage.version != currentversion) {
                ...
                chrome.browserAction.setBadgeText(f),
                count = 1,
        ...
        case "disableShowFeatureBar":
            ...
            break;
        case "getAD":
            $.get("http://api.hostip.info/get_json.php",
            function (b) {
                var d = b.ip,
                e = encodeURIComponent(a.url),
                f = a.query.replace(/\s/g, "+"),
                g = encodeURI(window.navigator.userAgent),
                h = "http://65975.xml.premiumxml.com/xml/?fid=65975&keywords=" + f + "&user_ip=" + d + "&ua=" + g + "&serve_url=" + e;
                $.get(h,
                function (a) {
                    var b = a,
                    d = b.getElementsByTagName("listing"),
                    e = d.length;
                    if (e) {
                        for (var f = [], g = 0; e > g; g++) {
                            var h = JSON.parse(xml2json(d[g], ""));
                            f.push(h.listing)
                        }
                        c({
                            data: f
...
chrome.tabs.onUpdated.addListener(function (a, b, c) {
    "chrome-extension://alelhddbbhepgpmgidjdcjakblofbmce/#" == b.url && (chrome.tabs.remove(c.id), chrome.extension.sendRequest({
        name: "loginByGoogle"
    })),
