function prepareEditArea(a) {
    ...
    function c(a, b, d, e, f, h, i, l, m) {
        h = j * k,
        j == s - 1 && (b = k - lastW, e = l = lastW),
        $("#save-image").attr({
            src: a
        }).load(function () {
            $(this).unbind("load"),
            showCtx.drawImage(this, b, d, e, f, h, i, l, m),
            s - 1 > j && c(g[++j], b, d, e, f, h, i, l, m)
        })
    }
    function d() {
    ...
    switch (f) {
        ...
        case "entire":
            ...
            if (!p.x && p.y) {
                ...
                t = r,
                lastH = l * o.y,
                "selected" == e ? (p.realX && (l -= scrollbarWidth), editW = a.centerW * window.devicePixelRatio) : editW = k,
            ...
            if (p.x && !p.y) {
                ...
                s = r,
                lastW = k * o.x,
                "selected" == e ? (p.realY && (k -= scrollbarWidth), editH = a.centerH * window.devicePixelRatio) : editH = l,
            ...
            if (p.x && p.y) {
                ...
                k -= scrollbarWidth,
                l -= scrollbarWidth,
                "selected" == e ? (editW = a.centerW * window.devicePixelRatio, editH = a.centerH * window.devicePixelRatio) : (editW = lastW ? k * (s - 1) + lastW : k * s, editH = lastH ? l * (t - 1) + lastH : l * t),
                updateEditArea(),
                updateShowCanvas();
                var h = 0,
                u = dw = k,
...
function prepareTools() {
    ...
    $("#launch-app").unbind().click(function () {
        if (isAppInstalled) {
            ...
            chrome.runtime.sendMessage("afkccfnochoebimhhniekgcegeeiepmi", {
                name: "launch",
                dataUrl: a,
                title: tabtitle
            })
        } else chrome.tabs.create({
            url: "https://chrome.google.com/webstore/detail/awesome-screenshot-app/afkccfnochoebimhhniekgcegeeiepmi"
        })
    }),
    $("#tool-panel>div").click(function (a) {
        function b(a) {
            var c = a.nodeName;
            return "A" != c && "DIV" != c && (a = a.parentNode, b(a)),
            a
        }
        var c = b(a.target);
        console.log(c),
...
function preparePromote() {
    $("#promote").click(function (a) {
        ...
        "A" != a.target.tagName && $(this).toggleClass("expanded").find("#content").toggle()
    })
}
...
function tool(a) {
    if (drawCanvas.width * drawCanvas.height != 0 && "color" != a && "done" != a && "cancel" != a) {
        if ("undo" == a) return $("body").hasClass("draw_free_line") || $("body").hasClass("draw_text_highlight") ? undo() : $(drawCanvas).attr({
            width: 0,
            height: 0
        }).unbind().css({
            left: 0,
        ...
        void (0 == actions.length && disableUndo());
        $("body").hasClass("draw_free_line") || $("body").hasClass("draw_text_highlight") || (saveAction({
            type: "draw"
    ...
    switch ("color" != a && (saveText(), "undo" != a && "resize" != a && ($("#temp-canvas").remove(), $("body").removeClass("justCropped draw draw-text draw-blur"))), updateBtnBg(a), a) {
        case "save":
            save();
            break;
        case "crop":
...
function save() {
    ...
    function b() {
        function a() {
            localStorage.format && !isPngCompressed && (l.image_type = localStorage.format),
            e = $.ajax({
                ...
                success: function (a, d, e) {
                    $("#loader").hide(),
                    200 == e.status && 1 == a.code ? (b(a.result.url), isGASafe && _gaq.push(["_trackEvent", "SavePageActions", "upload_success", "time"])) : c()
                },
                complete: function () { }
            })
        }
        function b(a) {
            $("#share-button, #email-link").show("slow").click(function (a) {
                ...
                var b = this;
                "buzz" == b.id && (b.href += "message=" + encodeURI(tabtitle) + "&url=" + encodeURI(taburl) + "&imageurl=" + a),
                "twitter" == b.id ? b.href = "http://twitter.com/share?url=" + encodeURIComponent(a) + "&via=awe_screenshot&text=" + tabtitle : $(b).attr({
                    href: b.href + a
                })
            }),
        ...
        function c() {
            $("#loader").hide("fast"),
            g || $("#error").show().find("#retry").unbind("click").click(function () {
                $("#error").hide(),
                $("#loader").show().find("a").unbind("click").click(d),
                a()
            })
        }
        function d() {
        ...
        m = "http://awesomescreenshot.com/client?";
        a(),
        window.showShare = b,
    ...
    $("#upload").unbind().click(b),
    $("#re-edit").unbind().text(chrome.i18n.getMessage("reEdit")).click(function () {
        return 1 == uploadFlag ? void $("#uploadingWarning").jqm().jqmShow() : ($("#saveOnline .content .diigo input[name=title]").val(""), $("body").removeClass("save"), $("#show-canvas").toggle(), $($editArea).disableSelection(), $("#share+dd div").hide(), $("#save_local+dd>p").hide(), $("#gdrive-share-link").hide(), void $(".sgdrive .saveForm").show())
    });
    var c = "";
    setTimeout(a, 100),
    window.uploadImageToAS = b,
    isSavePageInit || (SavePage.init(), isSavePageInit = !0)
}
function crop() {
    function a() {
        var a = document.getElementById("edit-area");
        ...
        var e, f, g, h;
        drawCtx.fillStyle = "rgba(80,80,80,0.4)",
        dragresize.ondragmove = function (a, i) {
            m.hide(),
            e = parseInt($("#cropdiv").css("top")),
            f = parseInt($("#cropdiv").css("left")),
            g = parseInt($("#cropdiv").css("width")),
            h = parseInt($("#cropdiv").css("height")),
            drawCtx.clearRect(0, 0, drawCanvas.width, drawCanvas.height),
            drawCtx.fillRect(0, 0, drawCanvas.width, drawCanvas.height),
            drawCtx.clearRect(f, e, g, h),
            b(e),
            c(g, h),
            d(i)
        },
        dragresize.ondragend = function () { }
    }
    function b(a) {
        n.css(30 > a ? {
            top: "5px"
    ...
    function c(a, b) {
        n.html(Math.abs(a) + " X " + Math.abs(b)),
    ...
    function d(a) {
        var b = a.clientY,
        c = k - b;
        80 > b && (document.body.scrollTop -= 25),
        40 > c && (document.body.scrollTop += 60 - c)
    }
    ...
    (cflag = 1) && (drawCtx.fillStyle = "rgba(80,80,80,0.4)", drawCtx.fillRect(0, 0, drawCanvas.width, drawCanvas.height)),
    cflag = 0;
    var e, f, g, h, i, j, k, l = mflag = 0,
    m = $("#crop-tip"),
    n = $("#crop_size").hide();
    $("#draw-canvas").hover(function () {
        m.text(chrome.i18n.getMessage("cropTip")).show()
    },
    function () {
        m.hide()
        ...
        return g = a.pageX - offsetX,
        h = a.pageY - offsetY,
        drawCtx.clearRect(0, 0, drawCanvas.width, drawCanvas.height),
        drawCtx.fillRect(0, 0, drawCanvas.width, drawCanvas.height),
        drawCtx.clearRect(e, f, g - e, h - f),
        d(a),
        l ? (i = g - e, j = h - f, mflag = 1, void c(i, j)) : void m.css({
            top: h + 5 + "px",
            left: g + 5 + "px"
        })
    }).mouseup(function (b) {
...
function color() {
    var a = null;
    $("#color").find("ul").fadeIn().hover(function (b) {
        $(this).show(),
        a && clearTimeout(a),
        b.stopPropagation()
    },
    function () {
        ...
        $("#text").hasClass("active") && $("div[contenteditable]").css({
            color: drawColor
        }),
        a.stopPropagation()
    })
...
function resize(a) {
    var b = parseInt(a),
    c = b / 100,
    d = showCtx.getImageData(0, 0, editW, editH);
    ...
    if (e > 1) for (var f = e - 1; f >= 0; f--) {
        var g = actions[f],
        h = g.type;
        if ("resize" == h && (0 == f || "resize" != actions[f - 1].type)) {
            d = g.data,
            editW = g.w,
            editH = g.h;
            break
...
function done() {
    ...
    $("#crop-tip").hide(),
    $("#crop-dimension").hide(),
    a = parseInt($("#cropdiv").css("top")),
    b = parseInt($("#cropdiv").css("left")),
    c = parseInt($("#cropdiv").css("width")),
    d = parseInt($("#cropdiv").css("height")),
    saveAction({
        type: "crop"
    });
    var a, b, c, d, e = showCtx.getImageData(b, a, c, d);
    $(showCanvas).attr({
        width: c,
        height: d
    }),
    showCtx.putImageData(e, 0, 0),
    $("body").removeClass().addClass("cropped justCropped"),
...
function undo() {
    function a() {
        ...
        getEditOffset(),
        updateShowCanvas(),
        showCtx.putImageData(c.data, 0, 0),
        c = null
    }
    var b = actions.length,
    c = actions.pop();
    if (0 != b) switch (1 == b && disableUndo(), c.f && ($("body").removeClass("cropped"), initFlag = 1), c.type) {
        case "draw":
            console.log("undo"),
            showCtx.putImageData(c.data, 0, 0);
            break;
        case "crop":
            ...
            break;
        case "resize":
            resizeFactor = c.factor,
            $("#resize select option").each(function () {
                $(this).text() == resizeFactor + "%" && $(this).siblings().removeAttr("selected").end().attr({
                    selected: "selected"
                })
            }),
            a()
...
function draw(a) {
    ...
    "free-line" == a ? ($("body").addClass("draw_free_line"), isHighlight = !1, $(showCanvas).unbind(), $("#temp-canvas").length || createTempCanvas(), void freeLine()) : "text-highlighter" == a ? ($("body").addClass("draw_text_highlight"), $(showCanvas).unbind(), $("#temp-canvas").length || createTempCanvas(), isHighlight = !0, void freeLine()) : ($(drawCanvas).unbind("mousedown"), "blur" == a ? ($("body").addClass("draw-blur"), void blur()) : ("text" == a && $("body").addClass("draw-text"), void $(showCanvas).unbind().click(function (b) {
        ...
        drawCanvas.width * drawCanvas.height != 0 && (saveAction({
            type: "draw"
        }), showCtx.drawImage(drawCanvas, parseInt($(drawCanvas).css("left")), parseInt($(drawCanvas).css("top")))),
        $(drawCanvas).attr({
            width: 0,
...
function shape(a, b) {
    function c(b, c) {
        function f() {
            drawCtx.clearRect(0, 0, o, q),
            drawCtx.strokeRect(j, j, o - 2 * j, q - 2 * j)
        }
        function g() {
            function a(a, b, c, d) {
                var e = c / 2 * .5522848,
        ...
        function h() {
            function a(a) {
                drawCtx.beginPath(),
                drawCtx.moveTo(a[0][0], a[0][1]);
                for (p in a) p > 0 && drawCtx.lineTo(a[p][0], a[p][1]);
                drawCtx.lineTo(a[0][0], a[0][1]),
                drawCtx.fill()
            }
            function b(a, b, c) {
                var d = [];
                for (p in a) d.push([a[p][0] + b, a[p][1] + c]);
                return d
            }
            function c(a, b) {
                var c = [];
                for (p in a) c.push(f(b, a[p][0], a[p][1]));
                return c
            }
            function f(a, b, c) {
                return [b * Math.cos(a) - c * Math.sin(a), b * Math.sin(a) + c * Math.cos(a)]
            }
            drawCtx.clearRect(0, 0, o, q),
            drawCtx.beginPath();
            var g = k > d ? j : o - j,
            ...
            my1 = q - h,
            drawCtx.moveTo(g, h),
            drawCtx.lineTo(i, my1),
            drawCtx.stroke();
            var m = [[4, 0], [-10, -5.5], [-10, 5.5]],
            n = Math.atan2(my1 - h, i - g);
            a(b(c(m, n), i, my1))
        }
        function i() {
            drawCtx.clearRect(0, 0, o, q),
        ...
        switch ($(drawCanvas).attr({
            ...
            top: n + "px",
            cursor: "crosshair"
        }).disableSelection(), drawCtx.strokeStyle = drawColor, drawCtx.fillStyle = drawColor, drawCtx.lineWidth = j, a) {
            case "rectangle":
                f();
                break;
            case "ellipse":
...
function freeLine() {
    $(drawCanvas).attr({
    ...
    }).disableSelection().off("mousedown mouseup").mousedown(function (a) {
        saveAction({
            type: "draw"
        });
        var b = document.getElementById("temp-canvas"),
        c = b.getContext("2d"),
        d = a.pageX - offsetX,
        e = a.pageY - offsetY,
        f = [];
        ...
        $(this).mousemove(function (a) {
            ...
            c.beginPath();
            for (var d = 0; d < f.length; d++) c.lineTo(f[d].x, f[d].y),
            c.lineJoin = "round",
            c.lineCap = "round",
            c.strokeStyle = isHighlight ? highlightColor : drawColor,
            c.lineWidth = isHighlight ? highlightWidth : 3,
...
function blur() {
    function a(a, b) {
        var c, d, e, f, g = a.width,
        h = a.height,
        i = a.data,
        j = b || 0,
        k = step = jump = inner = outer = arr = 0;
        for (f = 0; j > f; f++) for (var l = 0; 2 > l; l++) for (l ? (outer = g, inner = h, step = 4 * g) : (outer = h, inner = g, step = 4), c = 0; outer > c; c++) for (jump = 0 === l ? c * g * 4 : 4 * c, e = 0; 3 > e; e++) {
            for (k = jump + e, arr = 0, arr = i[k] + i[k + step] + i[k + 2 * step], i[k] = Math.floor(arr / 3), arr += i[k + 3 * step], i[k + step] = Math.floor(arr / 4), arr += i[k + 4 * step], i[k + 2 * step] = Math.floor(arr / 5), d = 3; inner - 2 > d; d++) arr = Math.max(0, arr - i[k + (d - 2) * step] + i[k + (d + 2) * step]),
            i[k + d * step] = Math.floor(arr / 5);
            arr -= i[k + (d - 2) * step],
            i[k + d * step] = Math.floor(arr / 4),
            arr -= i[k + (d - 1) * step],
            i[k + (d + 1) * step] = Math.floor(arr / 3)
        }
        return a
    }
    $(showCanvas).unbind().mousedown(function () {
        saveAction({
            type: "draw"
        }),
        $(this).mousemove(function (b) {
            var c = b.pageX - offsetX,
            d = b.pageY - offsetY,
            e = showCtx.getImageData(c, d, 20, 20);
            e = a(e, 1),
            showCtx.putImageData(e, c, d),
            $("body").hasClass("blurBugFix") ? $("body").removeClass("blurBugFix") : $("body").addClass("blurBugFix")
        })
    }).mouseup(function () {
...
function text(a) {
    function b() {
        $('<input class="textinput"></input>').appendTo($editArea).css({
            top: c + "px",
            left: d + "px",
            width: e + "px",
            color: drawColor
            ...
            if ($(this).width() + 10 > f && a.keyCode >= 48 || parseInt($(this).css("top")) - startT + 38 > g && 13 == a.keyCode) return !1;
            var d = a.target,
            e = a.keyCode;
            13 == e && (c += 18, b()),
            8 == e && (d.value || ($(d).prev().prev().focus().end().end().next().remove().end().remove(), c -= 18)),
            38 == e && $(d).prev().prev().focus(),
            40 == e && $(d).next().next().focus(),
            a.stopPropagation()
        })
    }
    saveText(),
    $("body").addClass("draw-text");
    var c = startT = a.y - offsetY - 10,
    d = a.x - offsetX;
    d > editW - e ? d = editW - e : "";
    var e = 20,
    f = editW - d,
    g = editH - c;
    1 == textFlag && b(),
    2 == textFlag && (textFlag = 1)
}
function saveText() {
    console.log("1");
    var a = $($editArea).find('input[class="textinput"]');
    if (console.log(a), a.length) {
        ...
        a.each(function () {
            ...
            if (b) {
                ...
                showCtx.fillStyle = $(a).css("color"),
                showCtx.fillText(b, c, d + 14)
            }
            console.log("2"),
            $(a).next().remove().end().remove()
        })
    }
}
...
function getLocVersion() {
    ...
    return a.open("GET", "./manifest.json", !1),
    a.send(null),
    JSON.parse(a.responseText).version
}
function addMargin() {
...
function showInfo(a) {
    if (a) var b = '<div class="w-info" id="w-cpy-info"><div class="w-info-topBar"><div class="w-info-logo"></div><div>Awesome Screenshot</div></div><p>It has been announced that Chrome will no longer allow extensions to make use of NPAPI, and therefore extensions will no longer be able to provide "copy image to clipboard" feature.   The current "Copy" feature is a workaround. If It doesn\'t work for you, please follow the instruction below:</p><p>Right-click the image in the left pane, and then select "<b>Copy Image</b>". </p><div class="w-close-btn"></div></div>';
    else var b = '<div class="w-info" id="w-cpy-info"><div class="w-info-topBar"><div class="w-info-logo"></div><div>Awesome Screenshot</div></div><p>Previously, Awesome screenshot  was able to make use of plugins through a standard plugin system called NPAPI. It has been <a href="http://blog.chromium.org/2013/09/saying-goodbye-to-our-old-friend-npapi.html" target="_blank">announced</a> that Chrome will no longer allow extensions to make use of NPAPI, and therefore extensions will no longer be able to provide "copy image to clipboard" feature. </p><p>After some hard work, we finally worked around this issue and enable the copy feature for you again. </p><p>We are sorry for this change.  If you like awesome screenshot, please <a href="https://chrome.google.com/webstore/detail/awesome-screenshot-captur/alelhddbbhepgpmgidjdcjakblofbmce/reviews">give it a nice 5-star rating</a>. Thanks for the support!</p><div class="w-close-btn"></div></div>';
    var c = '<div class="w-wrapper"></div>',
    d = $(c).appendTo(document.body).css({
        visibility: "visible",
        opacity: 1
    }),
    e = $(b).appendTo(document.body);
    e.find(".w-close-btn").on("click",
    function () {
        d.remove(),
        e.remove()
    })
...
SavePage.request = function (a, b, c) {
    ...
    b = JSON.stringify(b),
    d = "cv=" + f.cv + "&ct=" + f.ct + "&v=" + f.v + "&cmd=" + a + "&json=" + encodeURIComponent(b) + "&s=" + hex_md5("" + f.ct + f.cv + b + f.v + a) + d,
    e = new XMLHttpRequest,
    e.open("POST", f.url + ("/pv=" + f.pv + "/ct=" + f.ct), !0),
    e.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"),
    e.setRequestHeader("X-Same-Domain", "true"),
...
SavePage.updateUserInfo = function () {
    if (localStorage.user_info) {
        ...
        $("#accountInfo .name").attr("href", "https://www.diigo.com/user/" + a + "?type=image").html(a),
    ...
    } else $("#saveOptionContent>.diigo").removeClass("signin"),
    $(".share, .saveForm, .premium", $(".diigo")).hide()
},
SavePage.handleUserInfo = function (a) {
    localStorage.user_info = JSON.stringify(JSON.parse(a.response).result),
...
SavePage.signout = function () {
    ...
    a.setAttribute("src", "https://www.diigo.com/sign-out"),
        ...
        switch (a.name) {
            case "loginByGoogle":
                ...
                function (a) {
                    ...
                    SavePage.loadUserInfo(JSON.parse(a.response).user_id)
                })
        }
    })
},
SavePage.loginByDiigo = function () {
    function a() {
        var a = !1;
        return b && c ? a = !0 : b && !c ? $("#account input[name=password]").focus().addClass("empty") : !b && c ? $("#account input[name=username]").focus().addClass("empty") : ($("#account input[name=username]").focus().addClass("empty"), $("#account input[name=password]").addClass("empty")),
        a
    }
...
SavePage.initAccount = function () {
    localStorage.user_info ? SavePage.loadUserInfo(JSON.parse(localStorage.user_info).info.user_id) : SavePage.updateUserInfo(),
...
SavePage.showUploadResponse = function (a, b) {
    function c() {
        ...
        $(".shareLink", $("." + a)).find("input[type=text]").val(d).bind("mouseup",
        function () {
            $(this).select()
        })
    }
    ...
    "diigo" === a ? $("#privacy").is(":checked") ? (d = b.url, $(".diigo .privateLink").attr({
        href: d
    }), $(".share", $("." + a)).removeClass("public").addClass("private")) : (d = b.image_share_url, c(), $(".share", $("." + a)).removeClass("private").addClass("public")) : (d = b.url, c()),
    $(".share", $("." + a)).show(400)
},
SavePage.uploadImageToAS = function () {
    ...
    b.open("POST", c.url + "cmd=" + c.cmd + "&pv=" + c.pv + "&ct=" + c.ct + "&cv=" + c.cv, !0),
    b.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"),
    b.setRequestHeader("X-Same-Domain", "true"),
    b.onreadystatechange = function () {
        4 == this.readyState && (SavePage.response(b,
        function (a) {
...
SavePage.uploadImageToDiigo = function () {
    ...
    function (b) {
        ...
        d = c.permission;
        localStorage.user_info = JSON.stringify(c),
        (d.is_premium || d.image) && SavePage.request("uploadItems", a,
        function (a) {
            ...
            if (4 == this.readyState) {
                switch (uploadFlag = !1, c.status) {
                    case 200:
                        ...
                        a.alternateLink && a.ownerNames && (0 == $("#gdrive-private").prop("checked") ? SavePage.setPublicGdrive(a.id) : $("#gdrive-share-link p").text("Image Link (Private, only you can view it.)"), $("#gdrive-user").show(), $(".loader").remove(), $("#gdrive-share-link input").val(a.alternateLink), $("#sccess-tip").show().delay(1e3).fadeOut(), $("#gdrive-share-link").show());
                        break;
                    case 401:
...
SavePage.initSaveOption = function () {
    ...
    $("#gdrive-user p span").bind("click",
    function () {
        $("#notice").show()
    }),
    localStorage.gdrive_current_user && ($("#gdrive-save").text("Save"), $("#gdrive-user").show(), $("#gdrive-user p span").text(localStorage.gdrive_current_user), $("#saveOptionList li.sgdrive span").text("(" + localStorage.gdrive_current_user + ")")),
    ...
    $(".btnDark").click(function (a) {
        $(a.target).hasParent("#authError") ? $("#saveOptionContent>.diigo").removeClass("signin") : "clear-authentication" == a.target.id && ($(".loader").remove(), $(".sgdrive .saveForm").show(), $("#gdrive-save").text("Connect and Save"), $("#notice").hide(), $("#gdrive-user").hide(), $("#saveOptionList li.sgdrive span").text(""), googleAuth.clear(), delete localStorage.gdrive_current_user, googleAuth = new OAuth2("google", gDriveConfig))
    }),
    $("#saveOptionList").click(function (a) {
        var b = a.target;
        $(b).hasParent("#saveOptionList") && ($("#saveOptionContent").find("." + b.className).addClass("selected"), $("#saveOptionHead, #saveOptionBody").addClass("showContent"), $("#saveLocal").hide())
    }),
